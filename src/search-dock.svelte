<script lang="ts">
    ...
    async function InputHelper() {
        let {data} = await fetchSyncPost("/api/query/sql",{stmt:`SELECT count(*) as total_num FROM blocks_fts WHERE blocks_fts MATCH 'content:${searchText}'`})
        total_num = data[0].total_num
        currentSearchResults = []
        for(let index = 0;index < total_num && index < max_search_num;index+=once_search_num){
            let result = await fetchSyncPost("/api/query/sql",{stmt:`SELECT blocks_fts.id as id, blocks_fts.content as content, blocks.content as doc  FROM blocks_fts join blocks WHERE blocks_fts.root_id = blocks.id and blocks_fts MATCH 'content:${searchText}' order by blocks_fts.root_id limit 20 offset 0`})
...
<div class="middle-search-dock">
    ...
    />
    <SearchList searchResults = {currentSearchResults}></SearchList>
